...
- **Schemas:** `shared/schemas/` already has response/WS-body schemas for
  ...
  `HumanReviewSchema`, `AlarmSchema`) were built specifically to double as
  WebSocket message bodies matching `FRONTEND_BACKEND_CONTRACTS.md`'s
  "Frontend Event Models" shapes. No schemas exist yet for users/roles/
  permissions, evidence/recordings/snapshots, calibration, config/settings,
  analytics, camera-tracking, or (despite first appearances — see §3.5's
  correction) the two event types `/ws/camera-health` actually needs.
...
### 3.3 Incident lifecycle mutations must go through `IncidentService`, not direct repository writes

**Found during Architecture Readiness Review, fixed here.** §3.1's router
table lists `PATCH /incidents/{id}` as an `incidents.py` route without
specifying its write path. Checked directly against
`services/incident_service/service.py`: `IncidentService`'s own docstring
states it "owns incident creation, state transitions, persistence,
publication, idempotency, and lifecycle management," and its only status-
transition method, `_transition()`, is private — called exclusively from
`handle_escalation()` and `sweep_track_lost()`, both internal, pipeline-
driven entry points. **No public method exists today for an
operator-initiated status change.** A `PATCH /incidents/{id}` route that
wrote to the `incidents` table directly via `apps.api.app.repositories.
incident.IncidentRepository` would create two independent writers to the
same lifecycle state — `IncidentService` (pipeline-driven) and `apps/api`
(operator-driven) — bypassing `docs/REPOSITORY_ARCHITECTURE.md`'s explicit
ownership rule (*"services/incident_service owns: Incident lifecycle"*)
and its "Clear ownership" / "No circular dependencies" principles.

**Decision:** `PATCH /incidents/{id}` must call into `IncidentService`
in-process — the same direct-call pattern `ThreatEngineRuntimeAdapter`
already uses (`apps/deepstream/app/threat_runtime_adapter.py` constructs
`IncidentService(session, bus)` and calls `handle_escalation()` directly;
no network/IPC boundary exists between these components, consistent with
this project's single-process architecture). This requires
`IncidentService` to gain one new **public** method for operator-initiated
transitions (e.g. `apply_operator_update(incident, new_status, actor_user_id)`),
kept distinct from the existing pipeline-driven `_transition()` path rather
than making `_transition()` itself public (the pipeline and an operator are
different callers with different validity rules — an operator should not be
able to trigger every transition a pipeline event can, and vice versa).

**Cross-subsystem note:** `services/incident_service` is owned by
`feature/incident-service`, not `feature/api` — this is a small, explicit,
additive method addition (not a redesign of the service), but per
`CLAUDE.md`'s Multi-Agent Collaboration rule ("Respect subsystem ownership.
Do not overwrite unrelated work.") it should be flagged and coordinated
rather than made silently from within an `apps/api`-scoped PR, even though
implementing it in the same PR is likely the pragmatic choice given how
small the change is. See the implementation plan's Phase 4 for the
concrete task.
...
**Correction found during Architecture Readiness Review:** §2 listed
`CameraHealthSchema` alongside the schemas that already double as WS
message bodies. That's inaccurate for `/ws/camera-health` specifically —
`CameraHealthSchema` (`camera_id, status, fps, last_frame_age_seconds`) is
the `GET /cameras/{id}/health` response shape, describing ongoing health
metrics, not a disconnect notification. `/ws/camera-health` actually needs
to forward **two** event types per `FRONTEND_BACKEND_CONTRACTS.md`
(`CameraDisconnectedEvent` and `SystemEvent`), and **neither has a
frontend-facing schema today** — `CameraDisconnectedPayload` (`camera_id,
reason`) and `SystemEventPayload` (`severity, source_component, message`)
exist internally (`shared/events/payloads.py`) but have no
`shared/schemas/` counterpart yet, unlike every other WS channel in this
plan. Two new schemas (`CameraDisconnectedSchema`, `SystemEventSchema`)
are required — called out explicitly in the implementation plan's Phase 5,
not silently assumed to already exist.

### 3.6 Response envelope

**REST success responses:** reuse `shared/schemas/api.py`'s existing
`ApiResponse[T]` — already the pattern `health.py` uses, already matches
`FRONTEND_BACKEND_CONTRACTS.md`'s `{"success": true, "data": {}}` contract
exactly. No change needed for the success case.

**REST error responses — found missing during Architecture Readiness
Review, defined here.** `ApiResponse[T]` has no `error` field
(`success: bool`, `data: T | None` only), and no exception handler,
`HTTPException` usage, or any other error-response mechanism exists
anywhere in `apps/api/` today (verified by direct search — zero hits).
RM-12 is the first milestone that will produce real REST failure modes
(401/403 from auth, 404 for missing resources, 409 for conflicting state
transitions, 422 for validation errors) across dozens of new routes — this
needs one consistent shape decided once, not invented ad hoc per router as
Phase 3/4 routers get written.

**Decision:** extend `ApiResponse[T]` with an additive, backward-compatible
field: `error: ApiError | None = None`, where `ApiError` is a new, small
model (`code: str`, `message: str`) in the same `shared/schemas/api.py`
module. Existing consumers (`health.py`'s current success responses) are
unaffected — `error` simply stays `None`. Pair this with one global FastAPI
exception handler (registered in `main.py`) that catches `HTTPException`
and validation errors and serializes them into
`ApiResponse(success=False, data=None, error=ApiError(...))`, so every
router gets consistent error formatting for free rather than each one
handling it independently. This is a small, additive change to `shared/`
(owned by `feature/shared-contracts`, same cross-subsystem consideration as
§3.3) — not a redesign of the envelope, and does not break
`FRONTEND_BACKEND_CONTRACTS.md`'s documented success shape.
