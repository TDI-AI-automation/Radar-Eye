...
| | |
|---|---|
| Current phase | Implementation Phase 1 (per `PROJECT_CONTEXT.md`) — RM-01, RM-02, RM-DEV, RM-06, RM-03, RM-04, and RM-07 merged to `develop`; RM-05 implemented on `feature/calibration`, pending review/merge |
| Repo stage | RM-01, RM-02, RM-DEV, RM-06, RM-03, RM-04, and RM-07 complete and merged into `develop` (commit `90c21cc`), following Principal Engineer review. RM-05 (Calibration Service) implemented on `feature/calibration` (synced with `develop` before implementation, since the branch had been stale since project bootstrap): `services/calibration/service.py` — `CalibrationService` (`calibrate()`, `estimate()`), `services/calibration/homography.py` (DLT ground-plane homography estimation/projection via numpy, no OpenCV dependency), per the RM-05 design review (see design note below). Also extends `CameraCalibrationRepository` with `get_latest_for_camera()`/`list_for_camera()`, and fixes a pytest module-collision issue (`--import-mode=importlib`) that would recur as more services add their own `test_service.py`. 24 new tests, 236 total passing repo-wide, 100% coverage; ruff/black/pre-commit clean; mypy advisory-only, 9 findings (accepted technical debt, unchanged). Not yet merged into `develop` — pending Principal Engineer review. `feature/shared-contracts`, `feature/api`, `feature/developer-infrastructure`, `feature/threat-engine`, `feature/incident-service`, and `feature/calibration` all retained as long-lived subsystem branches. All other `apps/*` and `services/*` packages remain stub `__init__.py` only — do not assume partial implementation exists anywhere else. |
...
**Current Milestone:**
RM-08 — Recording & Evidence Service (see `docs/IMPLEMENTATION_ROADMAP.md`)
...
**Note:** RM-05 is implemented on `feature/calibration` and passes all quality gates, but is pending Principal Engineer review/merge into `develop` — that review step is a separate action, not a blocker for starting RM-08.
...
| RM-05 | **Done** (implemented, pending review/merge into `develop`) | `feature/calibration` |
...
| Camera calibration service | `services/calibration/` | **In Progress** (RM-05 done, pending merge) | `feature/calibration` | Shared contracts, API service (persistence) |
...
| Date | Item |
|---|---|
| 2026-07-20 | RM-05 (Calibration Service) implemented on `feature/calibration`, per an explicit design review approved before implementation began (append-only calibration history/"latest wins" convention, and no reprojection-error acceptance gate -- both summarized in Snapshot's Design Note). `feature/calibration` was synced with `develop` first (43 commits behind, stale since project bootstrap). Delivers `services/calibration/service.py`: `CalibrationService` -- `calibrate()` (computes and persists a ground-plane homography from >=4 reference points, ADR-016), `estimate()` (projects an image point to distance + `DistanceZone` using a camera's latest calibration, raises `CalibrationNotFoundError` if none exists). `services/calibration/homography.py` implements DLT homography estimation/projection via numpy (no OpenCV dependency). Extends `CameraCalibrationRepository` with `get_latest_for_camera()`/`list_for_camera()`. 24 new tests (synthetic-geometry accuracy against `BENCHMARK_ACCEPTANCE_CRITERIA.md`'s tolerances of <=2m@20m/<=5m@50m, append-only history, missing-calibration error path, `CalibrationUpdatedEvent` publication against the real bus), 236 total passing repo-wide, 100% coverage. Also fixed a pytest module-collision issue (`--import-mode=importlib` in `pytest.ini`) that would have recurred as more services add their own `test_service.py`. Not yet merged into `develop` -- pending Principal Engineer review. |
