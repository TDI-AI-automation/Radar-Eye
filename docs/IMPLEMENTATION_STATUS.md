...
| | |
|---|---|
| Current phase | Implementation Phase 1 (per `PROJECT_CONTEXT.md`) — RM-01, RM-02, RM-DEV, RM-06, RM-03, and RM-04 merged to `develop`; RM-07 implemented on `feature/incident-service`, pending review/merge |
| Repo stage | RM-01, RM-02, RM-DEV, RM-06, RM-03, and RM-04 complete and merged into `develop` (commit `832cc6e`), following Principal Engineer review. RM-07 (Incident Service) implemented on `feature/incident-service`: `services/incident_service/service.py` — `IncidentService` (creation, dedup, state transitions, track-lost auto-close), per the RM-07 design review (see design note below). Also extends `IncidentRepository` with `get_active_for_track()` and fixes a mypy module-resolution issue that would recur as more services get their own test `conftest.py`. 10 new tests, 221 total passing repo-wide, 100% coverage; ruff/black/pre-commit clean; mypy advisory-only, 9 findings (accepted technical debt). Not yet merged into `develop` — pending Principal Engineer review. `feature/shared-contracts`, `feature/api`, `feature/developer-infrastructure`, `feature/threat-engine`, and `feature/incident-service` all retained as long-lived subsystem branches. All other `apps/*` and `services/*` packages remain stub `__init__.py` only — do not assume partial implementation exists anywhere else. |
...
**Current Milestone:**
RM-05 — Calibration Service (see `docs/IMPLEMENTATION_ROADMAP.md`)
...
**Blocking Issues:**
None. RM-05 depends on RM-02 and RM-03, both done and merged into `develop`.

**Note:** RM-07 is implemented on `feature/incident-service` and passes all quality gates, but is pending Principal Engineer review/merge into `develop` — that review step is a separate action, not a blocker for starting RM-05.
...
| RM-07 | **Done** (implemented, pending review/merge into `develop`) | `feature/incident-service` |
...
| Incident service (also owns the Alarm Service until it warrants its own subsystem — see RM-10) | `services/incident_service/` | **In Progress** (RM-07 done, pending merge) | `feature/incident-service` | Threat engine, API service |
...
| Date | Item |
|---|---|
| 2026-07-20 | RM-07 (Incident Service) implemented on `feature/incident-service`, per an explicit design review approved before implementation began (escalation-logic ownership and the deferred Threat Engine Runtime Adapter hand-off mechanism, summarized in Snapshot's Design Note). Delivers `services/incident_service/service.py`: `IncidentService` — `handle_escalation()` (idempotent create-or-return, no timing logic), `on_threat_assessment()` (metadata + track-lost bookkeeping from the real bus stream), `sweep_track_lost()` (10s auto-close, ADR-025). Extends `IncidentRepository` with `get_active_for_track()`. 10 new tests (creation + event publication, metadata caching, idempotency, the concurrent create-race fallback verified against a real `IntegrityError`, and the full track-lost sweep behavior), 221 total passing repo-wide, 100% coverage. Also fixed a mypy module-resolution issue (`explicit_package_bases`/`namespace_packages` in `pyproject.toml`) that would have recurred as more services add their own test `conftest.py`. Not yet merged into `develop` — pending Principal Engineer review. |
