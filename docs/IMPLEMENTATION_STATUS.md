...
| | |
|---|---|
| Current phase | Implementation Phase 1 (per `PROJECT_CONTEXT.md`) — RM-01, RM-02, RM-DEV, RM-06, RM-03, RM-04, and RM-07 merged to `develop` |
| Repo stage | RM-01, RM-02, RM-DEV, RM-06, RM-03, RM-04, and RM-07 complete and merged into `develop` (commit `90c21cc`), following Principal Engineer review. RM-07 (Incident Service) delivers `services/incident_service/service.py` — `IncidentService` (creation, dedup, state transitions, track-lost auto-close), per the RM-07 design review (see design note below). Also extends `IncidentRepository` with `get_active_for_track()` and fixes a mypy module-resolution issue that would recur as more services get their own test `conftest.py`. 10 new tests, 221 total passing repo-wide, 100% coverage; ruff/black/pre-commit clean; mypy advisory-only, 9 findings (accepted technical debt) — all re-verified clean on `develop` post-merge. `feature/shared-contracts`, `feature/api`, `feature/developer-infrastructure`, `feature/threat-engine`, and `feature/incident-service` all retained as long-lived subsystem branches. All other `apps/*` and `services/*` packages remain stub `__init__.py` only — do not assume partial implementation exists anywhere else. |
