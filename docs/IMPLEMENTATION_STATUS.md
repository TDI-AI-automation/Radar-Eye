...
| | |
|---|---|
| Current phase | Implementation Phase 1 (per `PROJECT_CONTEXT.md`) — RM-01, RM-02, RM-DEV, RM-06, RM-03, RM-04, RM-07, and RM-05 merged to `develop` |
| Repo stage | RM-01, RM-02, RM-DEV, RM-06, RM-03, RM-04, RM-07, and RM-05 complete and merged into `develop` (commit `29e0261`), following Principal Engineer review. RM-05 (Calibration Service) delivers `services/calibration/service.py` — `CalibrationService` (`calibrate()`, `estimate()`), `services/calibration/homography.py` (DLT ground-plane homography estimation/projection via numpy, no OpenCV dependency), per the RM-05 design review (see design note below). Also extends `CameraCalibrationRepository` with `get_latest_for_camera()`/`list_for_camera()`, and fixes a pytest module-collision issue (`--import-mode=importlib`) that would recur as more services add their own `test_service.py`. 24 new tests, 236 total passing repo-wide, 100% coverage; ruff/black/pre-commit clean; mypy advisory-only, 9 findings (accepted technical debt, unchanged) — all re-verified clean on `develop` post-merge. `feature/shared-contracts`, `feature/api`, `feature/developer-infrastructure`, `feature/threat-engine`, `feature/incident-service`, and `feature/calibration` all retained as long-lived subsystem branches. All other `apps/*` and `services/*` packages remain stub `__init__.py` only — do not assume partial implementation exists anywhere else. |
