"""Tests for apps.deepstream.app.bridge.AsyncBridge.

Uses a fake GLib main loop (threading.Event-backed run()/quit()) injected
via mainloop_factory -- the real gi/GLib bindings are provided by the
DeepStream/JetPack SDK and are not installed on this dev machine (confirmed
absent when this milestone's design review was written). start()/stop()
lifecycle and schedule()'s cross-thread contract are both fully exercised
without it.
"""
...
from apps.deepstream.app.bridge import AsyncBridge, BridgeNotRunningError
...
def test_default_mainloop_factory_requires_gi() -> None:
    """Confirms the deferred-import guard: constructing the default factory
    without the DeepStream/JetPack SDK's gi bindings fails clearly rather
    than at module import time."""
    from apps.deepstream.app.bridge import _default_mainloop_factory

    with pytest.raises(ModuleNotFoundError):
        _default_mainloop_factory()
