"""LiveStreamSignalingServer -- the local-only HTTP endpoint apps.api
proxies SDP offers to.

Bound to ``127.0.0.1`` only, never network-exposed (ADR-011,
"centralized access control" -- apps.api is the sole externally-reachable
authenticated door; see ``apps/api/app/routers/cameras.py``'s
``POST /cameras/{camera_id}/webrtc/offer``, which is what actually
authenticates the operator and forwards here).
...
its own before responding with the answer (see ``branch.py``'s
``handle_offer``). No persistent connection, no relay of individual ICE
candidates, appropriate for a single-node LAN deployment with host
candidates only.

``fastapi``/``uvicorn`` are already repository dependencies (used by
apps.api) -- this is the first time apps.deepstream imports them, not a
...
def build_app(offer_handler: OfferHandler) -> FastAPI:
    ...
    async def post_offer(camera_id: uuid.UUID, body: WebRtcOfferRequest) -> WebRtcAnswerResponse:
        try:
            ...
            raise HTTPException(404, f"No live stream branch for camera {camera_id}") from exc
...
class LiveStreamSignalingServer:
    """Runs the local-only FastAPI app on its own asyncio task, sharing
    the same event loop as the rest of apps.deepstream (uvicorn's
