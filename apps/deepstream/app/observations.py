...
RawDetection = tuple[int, str, float, tuple[float, float, float, float], int | None]
"""(class_id, label, confidence, (left, top, width, height), track_id).
Plain-primitive intermediate form -- what RuntimeAdapter's pyds-touching
extraction step reduces each ``NvDsObjectMeta`` to, before construction
crosses into this module (which has no pyds/Gst dependency at all)."""
...
def build_frame_observation(
    *,
    ...
    detections = tuple(
        ...
        for class_id, label, confidence, bbox, track_id in raw_detections
    )
