...
Engineering target for this implementation: **≤1.5s LAN glass-to-glass latency** -- not a universal guarantee, a target this design was measured against. Measured directly on real hardware, real browser, burned-in wall-clock overlay compared against actual capture-instant wall-clock time, across two independent validation rounds (2026-08-16): steady-state latency (checkpoints from ~2s through ~30s into a connection, 5+ independent samples per round) consistently landed in the **~0.8-1.2 second** range, meeting the target and roughly an order of magnitude better than HLS's measured ~10s. One elevated sample (~8.8s) was observed once, immediately at the start of a connection during a busier test run (several other checks executing in the same process moments before); it did not reproduce in a clean, isolated repeat of the same connection sequence, and mpegts.js's own `liveSync`/`liveBufferLatencyChasing` configuration is specifically designed to converge an initially-buffered connection down toward its target latency within a few seconds -- consistent with what was observed. Recorded here rather than omitted, per this project's own measurement-honesty standard: typical/steady-state behavior meets the target; an occasional elevated startup sample is possible and was not chased further as a separate investigation. Startup time (navigation to first decodable frame) measured ~0.7-1.3s across runs. Page reload and three simultaneous browser viewers both verified working across both rounds, with zero DeepStream pipeline-mutation log activity and CPU/RSS/GPU indistinguishable from zero-viewer baseline (same methodology as ADR-031's own validation) -- ADR-031's central architectural property (viewer lifecycle cannot touch the AI pipeline) holds under this transport exactly as it did under HLS. Browser hardware video decode could not be conclusively confirmed from this specific headless/sandboxed test environment (Playwright-launched Chromium with software GL, no `chrome://gpu` access, and `navigator.mediaCapabilities.decodingInfo()` reporting `powerEfficient: false` under those conditions) -- MSE+H.264 playback uses the same browser-native decode path as any other web video, so hardware acceleration is expected to engage under a real operator workstation's own browser/GPU policy exactly as it would for any other H.264 content, but this specific claim was not directly observable from within this sandbox and is not claimed as verified. `apps.api` now holds one live TCP relay connection per browser viewer per camera for the connection's duration (bounded by concurrent viewer count, not by AI/GPU load) -- a new but small and well-understood resource, unlike WebRTC's per-connection GStreamer state.
